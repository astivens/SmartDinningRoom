...
usabilidad y portabilidad) y pruebas de aceptación. La suite automatizada ejecuta
\textbf{196 pruebas con 100\% de éxito} y una cobertura global aproximada del
\textbf{80,4\%} de sentencias. El trabajo se apoya en estándares y técnicas reconocidas
de la ingeniería de software \parencite{ieee829,iso25010,myers2011,pressman2014}.
...
Se documentan los casos de uso más relevantes con la plantilla extendida (actor,
precondiciones, flujo principal, flujos alternativos y postcondiciones). Las reglas y
códigos de respuesta provienen de los controladores reales y están verificados por las
pruebas (Segunda Parte).

\subsubsection{CU-01 — Registrar estudiante}
\textbf{Actor:} Estudiante (no autenticado). \textbf{Endpoint:}
\texttt{POST /api/auth/register} (multipart). \textbf{Precondición:} el email no debe
existir.
\begin{description}[leftmargin=1.4em,style=nextline]
\item[Flujo principal] (1) El estudiante diligencia el formulario y adjunta documentos.
(2) Se valida la presencia de \texttt{archivoSisben}. (3) Se valida que el email no
exista. (4) Se parsea \texttt{diasComedor}. (5) Se crea el usuario (rol student,
contraseña cifrada) y el estudiante. (6) Respuesta 201.
\item[Alternativos] 2a. Falta el archivo SISBÉN $\rightarrow$ 400. 3a. Email ya
registrado $\rightarrow$ 400.
\item[Postcondición] Se crea un \texttt{User} (student) y un \texttt{Student} con
\texttt{isValidatedSisben = false}.
\end{description}

\subsubsection{CU-02 — Iniciar sesión}
\textbf{Actor:} Todos. \textbf{Endpoint:} \texttt{POST /api/auth/login}.
\begin{description}[leftmargin=1.4em,style=nextline]
\item[Flujo principal] (1) Envío de email, contraseña y rol. (2) Validación de campos.
(3) Búsqueda del usuario. (4) Verificación de contraseña (\texttt{bcrypt.compare}). (5)
Validación de \texttt{isActive}, rol e \texttt{isAuthorized}. (6) Emisión de tokens y
respuesta 200.
\item[Alternativos] 2a. Campos faltantes $\rightarrow$ 400. 3a. Usuario inexistente
$\rightarrow$ 401. 4a. Contraseña incorrecta $\rightarrow$ 401. 5a. Inactivo
$\rightarrow$ 401; rol no coincide $\rightarrow$ 403; no autorizado $\rightarrow$ 403.
Excepción no controlada $\rightarrow$ 500 (sin filtrar el stack trace).
\item[Postcondición] El cliente almacena los tokens para autenticar peticiones.
\end{description}

\subsubsection{CU-07 — Validar SISBÉN}
\textbf{Actor:} Administrador. \textbf{Endpoint:}
\texttt{POST /api/students/:id/validate-sisben}.
\begin{description}[leftmargin=1.4em,style=nextline]
\item[Flujo principal] (1) Solicitud de validación. (2) Extracción del texto del
documento SISBÉN (PDF con pdf-parse o imagen con OCR tesseract.js). (3) Normalización y
cotejo de cédula, nombre y apellido, verificando que la cédula aparezca en el documento
de identidad. (4) Si todo coincide, \texttt{isValidatedSisben = true} y respuesta 200.
\item[Alternativos] 1a. Estudiante no encontrado $\rightarrow$ 404. 3a. Inconsistencia
$\rightarrow$ \texttt{validated = false} con el detalle de cada \emph{mismatch}.
\item[Postcondición] Se actualiza el estado de validación y \texttt{sisbenValidationDetails}.
\end{description}

\subsubsection{CU-11 — Subir pago y comprobantes}
\textbf{Actor:} Estudiante. \textbf{Endpoints:} \texttt{POST /api/payments},
\texttt{POST /api/payments/upload}, \texttt{POST /api/payments/calculate}.
\begin{description}[leftmargin=1.4em,style=nextline]
\item[Flujo principal] (1) Se ingresa el monto y se calcula
$mealsIncluded = \lfloor monto/2000\rfloor$. (2) Se adjunta el comprobante. (3) Se crea
el pago con \texttt{isVerified = false} y respuesta 201.
\item[Alternativos] 1a. Monto $<2000 \rightarrow$ 400. 1b. Estudiante no encontrado
$\rightarrow$ 404. 2a. Sin archivo $\rightarrow$ 400.
\item[Postcondición] Queda un pago pendiente de verificación.
\end{description}

\subsubsection{CU-14 — Registrar almuerzo (caso de uso central)}
\textbf{Actor:} Supervisor. \textbf{Endpoint:} \texttt{POST /api/meals}.
\begin{description}[leftmargin=1.4em,style=nextline]
\item[Flujo principal] (1) Búsqueda/escaneo del estudiante (QR/cédula). (2) Validación de
existencia y estado activo. (3) Validación de día autorizado. (4) Validación de no haber
comido hoy. (5) Validación de disponibilidad de almuerzos. (6) Creación del
\texttt{MealAttendance}, incremento de \texttt{mealsUsed} y creación de
\texttt{SupervisorLog}. (7) Registro en \texttt{audit\_logs} y respuesta 201.
\item[Alternativos] 1a. No autenticado $\rightarrow$ 401. 2a. No encontrado
$\rightarrow$ 404; inactivo $\rightarrow$ 400. 3a. Día no autorizado $\rightarrow$ 400.
4a. Ya comió hoy $\rightarrow$ 400. 5a. Sin almuerzos $\rightarrow$ 400.
\item[Postcondición] Disminuye la disponibilidad; quedan trazas en
\texttt{meal\_attendances}, \texttt{supervisor\_logs} y \texttt{audit\_logs}.
\end{description}

\subsubsection{Otros casos de uso}
Por brevedad se resumen: \textbf{CU-12 Verificar pago} (\texttt{PATCH /api/payments/:id/verify};
404 si no existe; tras verificar, los \texttt{mealsIncluded} cuentan como disponibles);
\textbf{CU-09 Invitar/unir supervisor} (token válido 48 h; 400 ante campos faltantes,
token inválido o email duplicado); \textbf{CU-18 Calificar} (1--5 estrellas; 401 sin
autenticar, 404 si el estudiante no existe); \textbf{CU-19 Quejas} (anónimas o
identificadas; respuesta del admin con 404 si no existe); \textbf{CU-21 Auditoría}
(filtros validados por lista blanca; 400 ante filtros no permitidos).
...
La ejecución de \texttt{npm run test:coverage} produjo el resultado del
Código~\ref{lst:resultado}.

\begin{lstlisting}[caption={Resultado de ejecución de la suite},label={lst:resultado},captionpos=b]
